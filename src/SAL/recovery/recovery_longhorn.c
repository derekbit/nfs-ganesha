...
static pthread_rwlock_t v4_recov_rwlock = PTHREAD_RWLOCK_INITIALIZER;
...
static int longhorn_recov_init(void)
{
	...
	pthread_rwlock_wrlock(&v4_recov_rwlock);
	res = http_call(HTTP_POST, LONGHORN_RECOVERY_BACKEND_URL,
		payload, strlen(payload) + 1,
		&response, &response_size);
	pthread_rwlock_unlock(&v4_recov_rwlock);
	if (res != 0) {
				  ...
				  add_rfh_entry_hook add_rfh_entry)
{
	...
	pthread_rwlock_rdlock(&v4_recov_rwlock);
...
static void longhorn_add_revoke_fh(nfs_client_id_t *delr_clid, nfs_fh4 *delr_handle)
{
	...
	encoded_cid_recov_tag = url_encode(delr_clid->cid_recov_tag);
	assert(encoded_cid_recov_tag != NULL);

	encoded_rhdlstr = url_encode(rhdlstr);
